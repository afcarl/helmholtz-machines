...
class SemiBiHM(HelmholtzMachine):
    def __init__(self, p_y, p_h1, p_h2_layers, p_layers, q_layers, q_y, q_h2_layers, q_y_given_x, x_dim, y_dim ,sup_layer_size, unsup_layer_size, **kwargs):
        super(SemiBiHM, self).__init__(p_layers, q_layers, **kwargs)

        self.p_y = p_y 
        self.p_h1 = p_h1
        self.p_h2_layers = p_h2_layers
        self.p_layers = p_layers
        self.q_layers = q_layers
        self.q_y = q_y 
        self.q_h2_layers = q_h2_layers
        self.q_y_given_x = q_y_given_x
  
        self.children = [p_y] + [p_h1] + p_h2_layers + p_layers + q_layers + [q_y] + q_h2_layers + [q_y_given_x]
    ...
    def sup_lower_bound(self, features, label, n_samples):
        ...
        samples_layers, samples_y, samples_h2_layers, log_p_layers, log_p_y ,log_p_h1 , log_p_h2_layers, log_q_layers, log_q_y, log_q_h2_layers = self.sample_q_sup (x, label )
        ...
        log_p_h2_layers  = unflatten_values(log_p_h2_layers, batch_size, n_samples)
        ...
        log_q_h2_layers  = unflatten_values(log_q_h2_layers, batch_size, n_samples)
         
        prob_layes = tensor.zeros([batch_size, n_samples])
        for p in log_p_layers:
            prob_layes  =  p + prob_layes 
        
        prob_h2_layes = tensor.zeros([batch_size, n_samples])
        for p in log_p_h2_layers:
            prob_h2_layes  =  p +  prob_h2_layes
        ...
        prob_q_h2_layes = tensor.zeros([batch_size, n_samples])
        for p in log_q_h2_layers:
            prob_q_h2_layes = p + prob_q_h2_layes
    ...
    def unsup_lower_bound(self,features,  n_samples, samples_h1): 
        ...
        log_q_h2_layers  =  [v.reshape((batch_size, n_samples)) for v in log_q_h2_layers] 
    ...
    def sup_importance_weights(self,     log_p_layers,log_p_y ,log_p_h1 , log_p_h2_layers , log_q_layers, log_q_y, log_q_h2_layers,batch_size, n_samples):
        ...
        prob_all  = 0.5*(prob_layes + prob_h2_layes+ log_p_y + log_p_h1 +log_q_y - prob_q_layes - prob_q_h2_layes  ) -tensor.log(n_samples )
    ...
    def get_gradients(self, features, label, n_samples, mask):
        ...
        samples_layers_q_sup, samples_y_sup, samples_h2_layers_sup, log_p_layers_sup, log_p_y_sup ,log_p_h1_sup , log_p_h2_layers_sup, log_q_layers_sup, log_q_y_sup, log_q_h2_layers_sup = self.sample_q_sup ( x, y)
        
        w_sup = self.sup_importance_weights( log_p_layers_sup,log_p_y_sup ,log_p_h1_sup , log_p_h2_layers_sup , log_q_layers_sup, log_q_y_sup, log_q_h2_layers_sup, batch_size, n_samples)
	...
	"""
        ...
        samples_h2_layers_unsup =  [v.reshape((batch_size,v.shape[1]*n_samples)) for v in samples_h2_layers_unsup]   
        samples_h2_layers_sup =  [v.reshape((batch_size,v.shape[1]*n_samples)) for v in samples_h2_layers_sup]
        ...
        #-------------------------   
        log_p_h2_layers_sup = [v.reshape((batch_size, n_samples)) for v in log_p_h2_layers_sup]
        log_p_h2_layers_unsup = [v.reshape((batch_size, n_samples)) for v in log_p_h2_layers_unsup]
        ...
        #-------------------------        
        log_q_h2_layers_sup = [v.reshape((batch_size, n_samples)) for v in log_q_h2_layers_sup]
        log_q_h2_layers_unsup = [v.reshape((batch_size, n_samples)) for v in log_q_h2_layers_unsup]
        ...
        prob_h2_layes_unsup = tensor.zeros([batch_size, n_samples])
        for p in log_p_h2_layers_unsup:
            prob_h2_layes_unsup  =  p +  prob_h2_layes_unsup
        ...
        prob_q_h2_layes_unsup = tensor.zeros([batch_size, n_samples])
        for p in log_q_h2_layers_unsup:
        ...
        prob_h2_layes_sup = tensor.zeros([batch_size, n_samples])
        for p in log_p_h2_layers_sup:
            prob_h2_layes_sup  =  p +  prob_h2_layes_sup
        ...
        prob_q_h2_layes_sup = tensor.zeros([batch_size, n_samples])
        for p in log_q_h2_layers_sup:
            prob_q_h2_layes_sup = p + prob_q_h2_layes_sup
        ...
        cc_un =  tensor.concatenate([samples_h2_layers_unsup[0],samples_y_unsup ], axis=1)
        ...
        gradients_sup = merge_gradients(gradients_sup, p_h2_layers[-1].get_gradients_list(samples_h2_layers_unsup[-1], samples_h2_layers_sup[-1], weights=[w_unsup,w_sup ], alpha = alpha , beta = beta ,gamma=gamma))
