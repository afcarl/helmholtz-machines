...
class Supervised_BiHM(HelmholtzMachine):
    ...
    def sample_q_sup (self, features, label):
        ...
        # Sample upwards (q distributions)
    ...
    def sample_q_unsup(self, features, n_samples, h1):
        p_layers = self.p_layers #p(h1_i| h1_i+1)
        q_layers = self.q_layers
        p_h1 = self.p_h1 #p(h1| h2,y)
        p_y = self.p_y
        ...
        for l in xrange(n_unsup_layers):
            log_q_layers[l+1] = q_layers[l].log_prob(samples_layers[l+1], samples_layers[l])
        ...
        return h1, samples_y, log_p_layers, log_p_y, log_p_h1, log_q_layers, log_q_y
    ...
    def unsup_lower_bound(self,features, n_samples, samples_h1): 
        ...
        _, samples_y, log_p_layers, log_p_y, log_p_h1, log_q_layers, log_q_y = self.sample_q_unsup(x, n_samples, samples_h1)
    ...
    def log_likelihood(self, features, label, n_samples, mask):
        ...
        # sup_lower_bound == log B(x, y)
        # unsup_lower_bound == 2 log A(x)
	...
	"""
        ...
        samples_layers_q_unsup, samples_y_unsup, log_p_layers_unsup, log_p_y_unsup ,log_p_h1_unsup, log_q_layers_unsup, log_q_y_unsup = self.sample_q_unsup(x, n_samples, samples_layers_q_sup,y)
        ...
        log_p_h1_sup = log_p_h1_sup.reshape((batch_size, n_samples)) 
        ...
        cost= alpha * 0.5 * unsup_lower_bound + sup_lower_bound + beta * expectation + gamma * expectation_sup
        ...
        samples_y_sup   = flatten_values(samples_y_sup  , batch_size*n_samples)
        ...
        samples_layers_q_unsup = [flatten_values(samples_layers_q_unsup[v], batch_size*n_samples) for v in range(len(samples_layers_q_unsup))]
    
        gradients_unsup = OrderedDict()
        for l in xrange(n_unsup_layers ):
            gradients_unsup = merge_gradients(gradients_unsup, 
                    p_layers[l].get_gradients_list_h1( samples_layers_q_sup[l], samples_layers_q_sup[l+1], 
                    weights=[w_unsup, w_sup ], alpha = alpha , beta = beta ,gamma=gamma))
            gradients_unsup = merge_gradients(gradients_unsup,
                    q_layers[l].get_gradients_list_h1( samples_layers_q_sup[l+1], samples_layers_q_sup[l], 
                    weights=[w_unsup_q, w_sup_q], alpha = alpha , beta = beta ,gamma=gamma))
 
        gradients_unsup = merge_gradients(gradients_unsup,
                p_h1.get_gradients_list(samples_layers_q_sup[n_unsup_layers], samples_y_unsup,samples_layers_q_sup[n_unsup_layers], 
        ...
        return  expectation, Classification_term,log_q_y_sup[0], log_q_y_unsup[0], sup_lower_bound , 0.5*unsup_lower_bound, cost, total_lower_bound, gradients 
